        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">All Client need Reviewing</h3>
            </div>
            <c:if test="${css!=null}">
				...
				<c:forEach var="clientInfo" items="${objList.result}">
					<tr>
						...
						<td>
							...
							<c:if test="${clientInfo.clientState == 'reviewing' }">
								<spring:url value="/clientInfo/${clientInfo.id}/deny" var="denyUrl" />

   
								<button class="btn btn-danger" onclick="this.disabled=true;post('${denyUrl}', {denyReason:window.prompt('拒絕原因','') },'post')">審核失敗</button>
								<spring:url value="/clientInfo/${clientInfo.id}/ready" var="readyUrl" />
								<button class="btn btn-success" onclick="this.disabled=true;post('${readyUrl}')">審核通過</button>
							</c:if>
						</td>
					</tr>
				</c:forEach>
