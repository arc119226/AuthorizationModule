            ...
            <div class="box-body">
              <table id="developerTable" class="table table-bordered table-hover">
				...
				<c:forEach var="clientInfo" items="${objList.result}">
					<tr>
						...
						<td>
							<spring:url value="/myClientInfo/${clientInfo.id}" var="userUrl" />
							...
							<c:if test="${clientInfo.clientState == 'init' }">
								...
								<spring:url value="/myClientInfo/${clientInfo.id}/reviewing" var="reviewingUrl" />
								<button class="btn btn-primary" onclick="this.disabled=true;post('${reviewingUrl}')">送出審核</button>
							</c:if>
							
							<c:if test="${clientInfo.clientState == 'publish' }">
								<spring:url value="/myClientInfo/${clientInfo.id}/stop" var="stopUrl" />
								<button class="btn btn-danger" onclick="this.disabled=true;post('${stopUrl}')">停用</button>
							</c:if>
							
							<c:if test="${clientInfo.clientState == 'deny' }">
								<spring:url value="/myClientInfo/${clientInfo.id}/init" var="initUrl" />
								<button class="btn btn-info" onclick="this.disabled=true;post('${initUrl}')">重新編輯</button>
							</c:if>
							
							<c:if test="${clientInfo.clientState == 'ready' }">
								<spring:url value="/myClientInfo/${clientInfo.id}/publish" var="publishUrl" />
								<spring:url value="/myClientInfo/${clientInfo.id}/stop" var="stopUrl" />
							...
							<c:if test="${clientInfo.clientState == 'stop' }">
								<spring:url value="/myClientInfo/${clientInfo.id}/publish" var="publishUrl" />
								<spring:url value="/myClientInfo/${clientInfo.id}/init" var="initUrl" />
								<button class="btn btn-info" onclick="this.disabled=true;post('${initUrl}')">重新編輯</button>
								<button class="btn btn-success" onclick="this.disabled=true;post('${publishUrl}')">發佈</button>
              ...
              <ul class="pagination">
               <c:forEach var="i" begin="1" end="${(objList.totalCount/objList.pageSize)+1}" step="1">
			   		<li><a href="${pageContext.request.contextPath}/myClientInfo/list?pageNo=${i }">${i }</a></li>
			   </c:forEach>
			  </ul>
